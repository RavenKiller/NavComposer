...
class ActionClassifier(nn.Module):
    def __init__(self, model_name="facebook/dinov2-base", hidden_size=768):
        super().__init__()
        ...
        if "clip" in model_name:
            ...
            self.vit = AutoModel.from_pretrained(model_name, local_files_only=False)
        ...
        self.class_head = nn.Linear(hidden_size * 2, 3)
        self.criterion = nn.CrossEntropyLoss()

        for param in self.vit.parameters():
            param.requires_grad_(False)
        # For dinov2, train last layer
        total_layers = len(self.vit.encoder.layer)
        print("total layers: ", total_layers)
        for idx in [total_layers - 2, total_layers - 1]:
            for param in self.vit.encoder.layer[idx].parameters():
    ...
    def get_image_features(self, image):
        if "imagegpt" in self.model_name:
            return self.vit(input_ids=image).last_hidden_state
        else:
            return self.vit(pixel_values=image).last_hidden_state
    ...
    def forward(self, batch):
        ...
        logits = self.class_head(torch.cat((attn_res2, attn_res1), dim=1))
...
class Dinov2Action:
    def __init__(self, config):
        self.dinov2_config = config.action_config.dinov2_config
        self.model = ActionClassifier(
            model_name=self.dinov2_config.model_name,
            hidden_size=self.dinov2_config.hidden_size,
        )
        self.processor = AutoImageProcessor.from_pretrained(
            self.dinov2_config.model_name, local_files_only=False
        )
        self.post_process = self.dinov2_config.post_process
        if self.dinov2_config.ckpt_path:
            ckpt = torch.load(self.dinov2_config.ckpt_path, map_location="cpu")
            state_dict = {
    ...
    def __call__(self, first_image, second_image, post_process=0):
...
def train_and_evaluate():
    ...
    ckpt_path = ckpt_folder / "model_weights" / "best_dinov2_base.pth"
    ...
    model = ActionClassifier(model_name=model_name, hidden_size=hidden_size)
    if ckpt_old:
        accelerator.unwrap_model(model).load_state_dict(ckpt_old["state_dict"])
...
def inference():
    ...
    # seen_ds = ActionDataset(model_name=model_name, split="val_seen", need_infer=True)
